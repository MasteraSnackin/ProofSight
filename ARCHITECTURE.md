...
| Cognee | Active as local memory, recurrence lookup, adapter-ready JSONL, and optional official SDK/API bridge. Official ingestion is only active when Cognee is installed/configured. | The agent reads memory during report generation; reviewers see memory context in reports/dashboard; `cognee_adapter.py` can ingest the JSONL through Cognee SDK/API. | SQLite stores canonical inspection/action data. New findings are compared with prior findings, report Memory Context is generated, schema v2 records are appended to `traces/cognee_ingest_queue.jsonl`, and the optional adapter can call `cognee.remember(...)` or `add(...)`/`cognify(...)`. | Cognee fits long-term memory: repeated hazards, locations, actions and review outcomes. |
